"""
KAVE — Data Transformation Component
Feature engineering pipeline for the productivity prediction model.
"""
...
class DataTransformation:
    """Advanced feature engineering for the XGBoost productivity model."""

    def __init__(self):
        """Initialize paths and feature configuration."""
        ...
        self.features_to_keep = [
            ...
            "idle_impact", "idle_ratio", "incentive_target_ratio", "stress_index",
        ]

    def initiate_data_transformation(self, raw_path: str) -> str:
        """
        Run the complete feature engineering pipeline.

        Steps:
            1. Clean department names and fill missing WIP values
            2. Remove outlier incentives (>99th percentile)
            3. Extract temporal features (month, day, week)
            4. Compute derived features (stress_index, idle_ratio, etc.)
            5. Label-encode categorical columns
            6. Save processed data and encoding artifacts

        Args:
            raw_path: Path to the raw CSV data file.

        Returns:
            str: Path to the processed CSV output.
        """
        print("[Data Transformation] Advanced Feature Engineering...")
        ...
        print(f"[Data Transformation] Dataset saved. Rows: {len(processed_df)}")
